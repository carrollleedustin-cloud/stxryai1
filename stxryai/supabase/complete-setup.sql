-- ================================================================
-- StxryAI Complete Database Setup
-- Run this AFTER deleting all existing tables
-- This will create everything fresh and clean
-- ================================================================
...
-- Choices table (for interactive branching)
...
-- Function to update story stats when chapters change
CREATE OR REPLACE FUNCTION update_story_stats()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    UPDATE stories SET
      chapter_count = (SELECT COUNT(*) FROM chapters WHERE story_id = OLD.story_id),
      word_count = (SELECT COALESCE(SUM(word_count), 0) FROM chapters WHERE story_id = OLD.story_id)
    WHERE id = OLD.story_id;
    RETURN OLD;
  ELSE
    UPDATE stories SET
      chapter_count = (SELECT COUNT(*) FROM chapters WHERE story_id = NEW.story_id),
      word_count = (SELECT COALESCE(SUM(word_count), 0) FROM chapters WHERE story_id = NEW.story_id)
...
-- ================================================================
-- COMPLETE!
-- Database schema setup finished successfully
-- ================================================================
