...
const variants: Record<PrismButtonVariant, string> = {
  primary: 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white hover:shadow-[0_0_20px_rgba(139,92,246,0.5)] border-transparent',
  secondary: 'bg-white/5 border-white/10 text-white hover:bg-white/10 hover:border-white/20 backdrop-blur-md',
...
export function PrismButton({
  ...
  const base = 'relative inline-flex items-center justify-center gap-2 font-medium transition-all duration-300 border disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden group';
  
  const glowEffect = glow ? 'shadow-[0_0_15px_rgba(139,92,246,0.3)] hover:shadow-[0_0_25px_rgba(139,92,246,0.6)]' : '';

  const classes = [base, variants[variant], sizes[size], glowEffect, className].filter(Boolean).join(' ');
