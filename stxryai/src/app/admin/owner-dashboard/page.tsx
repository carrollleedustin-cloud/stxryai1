...
import { canAccessOwnerDashboard, getRoleDisplayName, getRoleBadgeColor } from '@/lib/auth/accessControl';
import { createClient } from '@/lib/supabase/client';

type OwnerTab = 'overview' | 'platform' | 'users' | 'financial' | 'system' | 'settings' | 'analytics' | 'security';
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [usersResult, storiesResult, subscriptionsResult] = await Promise.all([
        ...
        supabase.from('subscriptions').select('id, status, amount, created_at').eq('status', 'active'),
      ]);
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={`px-4 py-2 rounded-full text-sm font-bold border ${getRoleBadgeColor('owner')}`}>
              ...
              <div className="flex items-center gap-4 mt-4 md:mt-0">
                <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${
                  metrics.systemHealth === 'healthy' ? 'bg-green-500/10 border-green-500/30' :
                  metrics.systemHealth === 'warning' ? 'bg-yellow-500/10 border-yellow-500/30' :
                  'bg-red-500/10 border-red-500/30'
                }`}>
                  <span className={`w-2 h-2 rounded-full ${
                    metrics.systemHealth === 'healthy' ? 'bg-green-500 animate-pulse' :
                  ...
                  <span className={`text-sm ${
                    ...
                    System {metrics.systemHealth === 'healthy' ? 'Healthy' : metrics.systemHealth === 'warning' ? 'Warning' : 'Critical'}
                  </span>
                </div>
                <SpectralButton 
                  variant="primary" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <HolographicCard className="p-6">
                ...
                <p className="text-xs text-void-600 mt-1">
                  {metrics.totalUsers > 0 ? Math.round((metrics.premiumUsers / metrics.totalUsers) * 100) : 0}% conversion
                </p>
              </HolographicCard>
              ...
              <HolographicCard className="p-6">
                ...
                <div className="text-3xl font-bold text-void-100 mb-1">
                  ${(metrics.monthlyRevenue / 100).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </div>
                    ...
                    <div className="space-y-4">
                      {recentTransactions.length === 0 ? (
                        ...
                        recentTransactions.map((tx) => (
                          <div key={tx.id} className="flex items-center justify-between p-4 rounded-lg bg-void-900/30 border border-void-800/30">
                            <div>
                              <p className="font-medium text-void-200">{tx.users?.username || 'Unknown'}</p>
                              <p className="text-sm text-void-500">{tx.type} • {new Date(tx.created_at).toLocaleDateString()}</p>
                            </div>
                ...
                <GradientBorder>
                  <div className="bg-void-950/80 backdrop-blur-xl rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <HolographicCard className="p-6">
                        <h4 className="text-lg font-semibold text-void-200 mb-4">Revenue Breakdown</h4>
                        <div className="space-y-3">
                          <div className="flex justify-between">
                            <span className="text-void-400">Monthly Recurring</span>
                            <span className="font-bold text-void-100">${(metrics.monthlyRevenue / 100).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-void-400">Total Revenue</span>
                            <span className="font-bold text-void-100">${(metrics.totalRevenue / 100).toFixed(2)}</span>
                          </div>
                          <div className="flex justify-between">
                  ...
                  <div className="bg-void-950/80 backdrop-blur-xl rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-96 overflow-y-auto">
                          {systemLogs.length === 0 ? (
                            ...
                            systemLogs.map((log, idx) => (
                              <div key={idx} className="text-xs text-void-400 font-mono p-2 bg-void-800/30 rounded">
                                [{new Date(log.created_at).toLocaleTimeString()}] {log.message || log.type}
                              </div>
                            ))
                          )}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <SpectralButton variant="primary" onClick={() => handleSystemAction('backup')}>
                          <Icon name="ArrowDownTrayIcon" size={16} className="mr-2" />
                          Backup Database
                        </SpectralButton>
                        <SpectralButton variant="ghost" onClick={() => handleSystemAction('clear_cache')}>
                ...
                <GradientBorder>
                  <div className="bg-void-950/80 backdrop-blur-xl rounded-xl p-6">
                    <h3 className="text-xl font-semibold text-void-100 mb-6 capitalize">
                      {activeTab === 'platform' ? 'Platform Management' :
