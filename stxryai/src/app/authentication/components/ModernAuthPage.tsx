...
import { Mail, Lock, User, Eye, EyeOff, ArrowRight, Check, Sparkles, Github, Chrome } from 'lucide-react';
...
const ModernAuthPage = () => {
  ...
  const strengthColors = ['bg-spectral-rose', 'bg-plasma-orange', 'bg-spectral-amber', 'bg-spectral-emerald', 'bg-spectral-cyan'];
        ...
        >
          <div className="void-glass-heavy rounded-3xl p-8 md:p-10">
            ...
            <div className="space-y-3 mb-6">
              <button
                type="button"
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-void-mist border border-membrane hover:border-text-ghost transition-all duration-300 text-text-secondary hover:text-text-primary group"
              >
                ...
                {!isLogin && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-text-ghost">@</span>
                        <input
                          type="text"
                          value={username}
                      ...
                      {[0, 1, 2, 3, 4].map((i) => (
                        <div
                          ...
                          className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                            i < passwordStrength ? strengthColors[passwordStrength - 1] : 'bg-void-mist'
                          }`}
                        />
                      ))}
                    </div>
                    <p className={`text-xs font-ui ${
                      passwordStrength <= 1 ? 'text-spectral-rose' :
                      passwordStrength <= 2 ? 'text-plasma-orange' :
                      passwordStrength <= 3 ? 'text-spectral-amber' :
                  ...
                  >
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      ...
                      <span className="text-sm text-text-tertiary">
                        I agree to the{' '}
                        <Link href="/terms" className="text-spectral-cyan hover:underline">Terms</Link>
                        {' '}and{' '}
                        <Link href="/privacy" className="text-spectral-cyan hover:underline">Privacy Policy</Link>
                      </span>
                    </label>
                  </motion.div>
                )}
