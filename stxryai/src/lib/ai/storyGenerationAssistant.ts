...
export function generateCharacter(
  ...
  const template =
    protagonistTemplates[genre as keyof typeof protagonistTemplates] ||
    protagonistTemplates.fantasy;
...
export function generatePlotOutline(prompt: StoryPrompt): StoryOutline {
  ...
  const outline: StoryOutline = {
    ...
    estimatedWordCount:
      prompt.targetLength === 'short' ? 3000 : prompt.targetLength === 'long' ? 15000 : 8000,
  };
...
export function generateDialogue(
  ...
  return {
    ...
    alternatives: [dialogueLines.map((line) => line.replace('talk', 'discuss')).join('\n')],
    confidence: 0.75,
...
export function generateSceneDescription(
  ...
  const moodModifiers = {
    ...
    ominous:
      'Something felt wrong, though it was impossible to say what. Danger seemed to lurk just out of sight.',
  };
 ...
 */
export function generatePlotTwist(genre: string, currentPlot: string): GeneratedContent {
  ...
  const twist =
    twistTemplates[genre as keyof typeof twistTemplates] ||
...
export function analyzeContent(content: string, genre: string): ContentAnalysis {
  ...
  const descriptiveWords = (
    content.match(/\b(beautiful|dark|mysterious|ancient|gleaming)\b/gi) || []
  ).length;
  ...
  return {
    ...
    overall: {
      score: 75,
      strengths: ['Engaging premise', 'Clear narrative voice', 'Good genre conventions'],
      improvements: [
