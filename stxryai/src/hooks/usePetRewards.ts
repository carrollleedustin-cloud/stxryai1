...
export function usePetRewards(): PetRewardsHook {
  ...
  // Reading activities
  const onStoryRead = useCallback(
    async (storyId: string, genre?: string) => {
  ...
  const onChapterPublished = useCallback(
    async (storyId: string) => {
  ...
  // Social activities
  const onCommentWritten = useCallback(
    async (storyId: string) => {
  ...
  const onFriendMade = useCallback(
